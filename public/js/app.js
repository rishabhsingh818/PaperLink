...
async function api(path, opts={}){ const res = await fetch('/api'+path,{...opts, headers:{ Authorization:'Bearer '+token(), ...(opts.headers||{})}}); return res.json(); }
...
async function poll(){
  if(!token()) return; const d = await api('/notifications');
  (d.notifications||[]).slice(-3).forEach(n=>{
    const el = document.createElement('div'); el.className='glass item'; el.style.position='fixed'; el.style.right='16px'; el.style.bottom=(16 + Math.random()*60)+'px'; el.style.maxWidth='320px'; el.style.zIndex='50'; el.innerHTML = `<div class='meta'>${new Date(n.at).toLocaleTimeString()}</div><div>${n.text}</div>`; document.body.appendChild(el); setTimeout(()=>el.remove(), 3500);
  });
}
